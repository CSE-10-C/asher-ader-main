#Note for Mr. Jeffer's. This code aims to create a rudimentry text based adventure gam that utilizes coding concepts that we have only learned in class while still aiming to create a fun and ejoyable game for the user. 
...
inventory = { #User can acquire items, resources, and weapons throughout the game
    "food": 0, 
    "water": 0,
    "glock": False, #Becuase you either have it or you don't
    "ammo": 0,
    "medkit": 0,
...
alive = True #Goal is to staye alive and surivive the jungle. This thus determines a win or loss
escaped = False

def pause():#Allows the user to read before continuing on 
    input("\n[Press the Enter or Return key to continue] ") #/n just creates that new line #This helps it feel more like a adventure game rather then a story
...
def choice(prompt, options): #This prompts the user for a response until they give a valid option
    ...
    while True: 
        ...
        print("Pick one of the options above.") #Pompt user again if they dont give valid option
...
def start_scene(): # Now that all of the prerequisite stuff is made we can start the game. Everything here is not mosty a repitiion of the same concepts repeated until the game is fnished. Therefore, comments will be limited. 
    print("Youre roused suddenly to loud sounds, yelling, and uncontrollable movement .")
    print("Yu realize that your on a small plane diving straight down over a dense jungle. ")
    pause() #Lets them read before choosing to continue 
    print("The impact is a blur. Its loud and dark.")
    pause()
    print("You soon open your eyes and everything is silent.")
    print("Your strapped down to your seat in the middle of the jungle.")
...
def search_cockpit(): #If the user decides to search the cockpit
    print("\n You climb toward the cockpit of the plane doging dead boddies and bags.")
    print("The pilots are dead. You try not the look to closely.")
    print("Most the of the tools are broken. Nothing of use appears.")
    flare_chance = random.random()
    if flare_chance < 0.5: #50% chance you find a flare
        print("You managed to find a emergeny kit with a single flare gun in it.")
...
def eat_and_drink(): #Allows the user to use that food and water 
    print("\nYour overwrought. You sit for a moment to understand how you will move forward.")
    if inventory["food"] > 0: #So that they can only eat if they found food
        print("You eat some fod.")
...
def jungle_hub():
    ...
    print("The jungle surrounding you hums and you think you hear some sort of engine sounds. You figure its just from the plane right?")
    pause()
...
def river_scene():
    ...
    print("The water is rapid, brown, and dirty. You assume your somwhere in the amazon. Likly South America.")
    event = random.choice(["snake", "cartel", "quiet"])
    pause() #will encounter a random event

    if event == "snake":
        print("As your walking a colorful snake lunges at you from the bushes.")
        if inventory["glock"] and inventory["ammo"] > 0:
            options = {
                "1": "Try to aviod it", #if they have a glock with ammo they can use it 
                "2": "Use the Glock",
            }
            c = choice("What shall you do?", options)
            if c == "1": #if they choose to avoid it obv
                if random.random() < 0.6: #60% chance they can avoid it
                    print("You stumble away appearingly safe.")
                else:
                ...
                inventory["ammo"] -= 1
                print("You fire a shot. Its loud. The snake stops moving appearingly dead.")
                print("The gunshot is loud and echoes the forest. If somoene else is around they definitely heard it.")
        else:
            print("You jump back instinctly with your heart pounding.")
        ...
        print("It's very quite and you, thirsty, decide to refill your water bottle from the river.")
...
def ridge_scene(): #another contingent event
    ...
    if c == "1": 
        ...
        if inventory.get("flare", False):
            ...
            print("Helps arrived! Your spotted and saved.")
            escaped = True
        else:
            print("\nYou search your pockets.No flare to call for help.")
        pause()
    elif c == "3":
        print("\nYou decide its safer to go below the near tree line.")
        pause()
...
def cartel_encounter(location): #basic cartel encounter that gives that engages the user
    ...
    if location == "camp": #if there at the camp
        print("You spot a rough camp with armed men, fentynl, and a few trucks. In the Amazon maybe? Definitely cartel.")
    elif location == "river":
    ...
    if c == "1":
        print("\nYou duck behind the thick bushes and be sure not to move a inch.")
        ...
        print("You carefully follow them at a distance, careful to aviod making any noise.")
        pause()
        airstrip_scene()

def airstrip_scene():
    global alive, escaped
    print("\nAfter some time following them you finally reach and airstrip surrounded by the jungle.")
    ...
    if c == "1":
        ...
        print("The guards relax and fall asleep under the darknes of night.")
        if random.random() < 0.7: #70 Percent change
        ...
        if inventory["ammo"] < 0: #straitforward
        ...
        if random.random() < 0.6:
            ...
            print("The engine doesnt start. Cheap plane ig.")
...
def ending():
    ...
    if escaped and alive:
        ...
        print("This is where your story ends. You didnt make it out. Im sorry Mr. Jeffers")
    else:
